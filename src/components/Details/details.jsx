  ...
  if (!product) {
    return (
      <div className="details__not-found">
        ...
        <button className="details__back-btn" onClick={() => navigate('/')}>
          ← Back to Products
        </button>
      </div>
    )
    ...
    <div className="details">
      ...
      <div className="details__breadcrumb">
        <div className="details__breadcrumb-inner">
          <button className="details__breadcrumb-link" onClick={() => navigate('/')}>
            Home
          </button>
          <span className="details__breadcrumb-sep">/</span>
          <span className="details__breadcrumb-link" onClick={() => navigate('/')}>
            Products
          </span>
          <span className="details__breadcrumb-sep">/</span>
          <span className="details__breadcrumb-current">{product.name}</span>
        </div>
      </div>
          ...
          <div className="details__info-col">
            ...
            <div className="details__meta">
              <div className="details__meta-item">
                ...
                <span className="details__meta-value">{product.category}</span>
              </div>
              <div className="details__meta-item">
                <span className="details__meta-label">Subcategory:</span>
                <span className="details__meta-value">{product.subcategory}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="details__tab-content">
            {activeTab === 'description' && (
              <div className="details__tab-panel">
                <p className="details__tab-text">{product.description}</p>
                {product.features && (
                  <ul className="details__features">
                    {product.features.map((f, i) => (
                      <li key={i} className="details__feature">
                ...
                <table className="details__spec-table">
                  <tbody>
                    {Object.entries(product.additionalInfo).map(([key, value]) => (
                      <tr key={key}>
                        <td className="details__spec-key">{key}</td>
                        <td className="details__spec-value">{value}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {relatedProducts.length > 0 && (
        <section className="details__related">
          <div className="details__related-inner">
            <h2 className="details__related-title">Related Products</h2>
